...
2.  **Tmpfs Isolation**: `/tmp/nixos-build` 디렉터리를 생성하고 소스 파일을 복사합니다. 하드디스크가 아닌 RAM 디스크를 사용하므로 I/O 속도가 빠르고 메인 저장소를 오염시키지 않습니다.
...
1.  **Metadata Parsing**: `flake.nix`가 `dev/_info.json`을 읽어 모든 호스트 설정을 AttrSet으로 생성합니다.
