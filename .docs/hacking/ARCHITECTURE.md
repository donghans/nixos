...
- **`nixup.sh` (Dispatcher)**:
  ...
  - **특징**: `nix-shell` 쉬뱅을 사용하여 `jq`, `nom` 등의 도구가 없어도 시스템을 부트스트랩할 수 있도록 설계되었습니다. 로깅(`YYYYMMDDTHHMMSS-[scope]-[action].log`, 예: `20260405T120000-os-switch.log`)과 세션 락(`flock`)을 독점적으로 관리합니다.
- **Task & Lib**:
