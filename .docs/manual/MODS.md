...
> 내부 작동 원리(모듈 스캐닝, enable 계층, Dual-Context 등)는 [ARCHITECTURE-MODS.md](../hacking/ARCHITECTURE-MODS.md) 참조
...
아래 예시들은 실제 프로젝트 모듈을 기반으로 한 패턴별 스니펫입니다. API 파라미터의 세부 사항은 다음 섹션 [API 레퍼런스](#api-레퍼런스)를 참조하세요.
