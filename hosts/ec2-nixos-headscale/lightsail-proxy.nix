{
  ...
  lightsailPubKey = pkgs.writeText "lightsail-headscale-proxy.pub"
    (builtins.readFile ../deploy/lightsail-headscale-proxy.pub);
...
in {
  ...
  systemd.services.lightsail-proxy-sync = {
    ...
    serviceConfig = {
      ...
      ExecStart = pkgs.writeShellScript "lightsail-proxy-sync"
