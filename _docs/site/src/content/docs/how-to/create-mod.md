...
> 내부 작동 원리(모듈 스캐닝, enable 계층, Dual-Context 등)는 [Mods 내부 원리](../explanation/mods-internals) 참조
> 헬퍼 함수 시그니처는 [Mod API](../reference/mod-api) 참조
...
아래 예시들은 실제 프로젝트 모듈을 기반으로 한 패턴별 스니펫입니다. API 파라미터의 세부 사항은 [Mod API](../reference/mod-api)를 참조하세요.
